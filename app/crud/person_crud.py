...
# --- Extra ---
async def get_person_by_name(name):
    persons = []
    async for p in collection.find({"name": name}):
        p["_id"] = str(p["_id"])
        persons.append(p)
    return persons

async def get_persons_by_start_letter(letter):
    persons = []
    regex = f"^{letter}"
    async for p in collection.find({"name": {"$regex": regex, "$options": "i"}}):
        p["_id"] = str(p["_id"])
        persons.append(p)
    return persons

async def delete_person_by_name(name):
